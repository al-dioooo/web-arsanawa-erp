...
export default function APAgingPage() {
    ...
    const { data: agingData = [], isLoading, isError, error, refetch } = useAPAging(activeCompanyId)
    ...
    return (
        <div className="w-full">
            ...
            <Card padding="lg" className="mb-6 flex items-start gap-4">
                <div className="rounded-md bg-error-soft p-3 text-error-strong">
                    <Icon name="account_balance_wallet" size={28} />
                </div>
                <div>
                    <h2 className="type-section mb-1">{t("totalOutstanding")}</h2>
                    <p className="type-card-value text-error-strong tabular-nums">{formatIDR(totals.total)}</p>
